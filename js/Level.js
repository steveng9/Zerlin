...
class Tile extends Entity {
  ...
  // Draw the floor's top surface as a trapezoid.  All three key y-positions
  // (backY, tile.y, frontY) are derived from the same formula:
  //   y = vanishY + (tileY - vanishY) * (100 / D)
  // and likewise for x.  This makes back/tile.y/front collinear — all on the
  // same ray from the vanishing point — so the sides of the trapezoid are
  // straight lines and the surface width at tile.y exactly matches the tile face.
  _drawFloorSurface(screenX) {
    ...
    // Back edge (D_back > 100): converged toward vanishX, above tile.y.
    ...
    // Front edge (D_front < 100): spread outward from vanishX, below tile.y.
    // D_front must be small enough that frontY lands below the canvas.
    var frontY     = vanishY + (this.y - vanishY) * (100 / D_front);
    ...
    // Gradient peaks at tile.y (the collision/face line — most lit).
    var tMid = (this.y - backY) / (frontY - backY);
    var grad = this.ctx.createLinearGradient(0, backY, 0, frontY);
